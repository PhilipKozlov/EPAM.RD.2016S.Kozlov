...
namespace DoSomethingClient
{
    public class DomainAssemblyLoader : MarshalByRefObject
    {
        ...
        public Result Load(string assemblyString, Input data)
        {
            ...
            IDoSomething doSomethingService = Activator.CreateInstance(FindFirstTypeWithDoSomethingAndIDoSomething(types)) as IDoSomething; // TODO Save instance to variable.
        ...
        public Result LoadFile(string path, Input data)
        {
            ...
            Type type = FindFirstTypeWithDoSomething(types); // TODO: Find first type that has DoSomething attribute and don't implement IDoSomething.
        ...
        private Type FindFirstTypeWithDoSomethingAndIDoSomething(Type[] types)
        {
            foreach (var type in types)
            {
                if (Attribute.IsDefined(type, typeof(DoSomethingAttribute)))
                {
                    if (type.GetInterfaces().Contains(typeof(IDoSomething))) return type;
                }
            }
            return null;
        }

        private Type FindFirstTypeWithDoSomething(Type[] types)
        {
            foreach (var type in types)
            {
                if (Attribute.IsDefined(type, typeof(DoSomethingAttribute)))
